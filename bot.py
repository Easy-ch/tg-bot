...
Для того, чтобы оформить заказ нужно отправить артикул товара и необходимый размер, 
также можно фотографию товара (инструкция о том, где можно найти артикул Poizon прикрепляем в видео 📌)
 @wnmarket1
  """)
def ans(message):
    global course
...
def course_change(message):
    bot.send_message(message.chat.id, "Введите новое значение курса: ")
